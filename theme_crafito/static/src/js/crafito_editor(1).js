odoo.define('theme_crafito.crafito_editor_js', function(require) {
    'use strict';
    ...
    ajax.loadXML('/theme_crafito/static/src/xml/theme_crafito.xml', qweb);

    options.registry.crafito_tabbing_snippet = options.Class.extend({
        start: function() {
            var self = this;
    ...
    options.registry.theme_crafito_blog_custom_snippet = options.Class.extend({
        start: function(editMode) {
            var self = this;
            this._super();
            this.$target.removeClass("o_hidden");
            this.$target.find('.crafito_blog_slider').empty();
            if (!editMode) {
                self.$el.find(".crafito_blog_slider").on("click", _.bind(self.theme_crafito_blog_slider, self));
            }
        },
        onBuilt: function() {
            var self = this;
            this._super();
            if (this.theme_crafito_blog_slider()) {
                this.theme_crafito_blog_slider().fail(function() {
                    self.getParent()._removeSnippet();
                });
            }
        },
        cleanForSave: function() {
            $('.crafito_blog_slider').empty();
        },
        theme_crafito_blog_slider: function(type, value) {
            var self = this;
            if (type != undefined && type.type == "click" || type == undefined) {
                self.$modal = $(qweb.render("theme_crafito.crafito_blog_slider_block"));
                ...
                ajax.jsonRpc('/theme_crafito/blog_get_options', 'call', {}).done(function(res) {
    ...
    options.registry.theme_crafito_multi_cat_custom_snippet = options.Class.extend({
        ...
        multi_category_slider: function(type, value) {
            var self = this;
            if (type != undefined && type.type == "click" || type == undefined) {
                self.$modal = $(qweb.render("theme_crafito.multi_product_custom_slider_block"));
                self.$modal.appendTo('body');
                self.$modal.modal();
                ...
                ajax.jsonRpc('/theme_crafito/product_multi_get_options', 'call', {}).done(function(res) {
    ...
    options.registry.theme_crafito_coming_soon_mode_one = options.Class.extend({
        start: function(editMode) {
            var self = this;
            this._super();
            this.$target.removeClass("o_hidden");
            ...
            if (!editMode) {
                self.$el.find(".biztech_coming_soon_mode_one").on("click", _.bind(self.theme_crafito_coming_soon_mode_one_function, self));
            }
        },
        ...
        onBuilt: function() {
            var self = this;
            this._super();
            if (this.theme_crafito_coming_soon_mode_one_function()) {
                self.theme_crafito_coming_soon_mode_one_function.fail(function() {
                    self.getParent()._removeSnippet();
                });
            }
        },
        ...
        theme_crafito_coming_soon_mode_one_function: function(type, value) {
            if (type != undefined && type.type == "click" || type == undefined) {
                var self = this;
                self.$modal = $(qweb.render("theme_crafito.model_dialog_commingsoon"));
                self.$modal.appendTo('body');
                self.$modal.modal();
    ...
    options.registry.theme_crafito_coming_soon_mode_two = options.Class.extend({
        start: function(editMode) {
            var self = this;
            this._super();
            this.$target.removeClass("o_hidden");
            ...
            if (!editMode) {
                self.$el.find(".biztech_coming_soon_mode_two").on("click", _.bind(self.theme_crafito_coming_soon_mode_two_function, self));
            }
        },
        ...
        onBuilt: function() {
            var self = this;
            this._super();
            if (this.theme_crafito_coming_soon_mode_two_function()) {
                self.theme_crafito_coming_soon_mode_two_function.fail(function() {
                    self.getParent()._removeSnippet();
                });
            }
        },
        ...
        theme_crafito_coming_soon_mode_two_function: function(type, value) {
            if (type != undefined && type.type == "click" || type == undefined) {
                var self = this;
                self.$modal = $(qweb.render("theme_crafito.model_dialog_commingsoon"));
                self.$modal.appendTo('body');
    ...
    options.registry.theme_crafito_category_slider = options.Class.extend({
        start: function(editMode) {
            var self = this;
            this._super();
            this.$target.removeClass("o_hidden");
        ...
        cat_slider: function(type, value) {
            var self = this;
            if (type != undefined && type.type == "click" || type == undefined) {
                self.$modal = $(qweb.render("theme_crafito.crafito_dynamic_category_slider"));
                self.$modal.appendTo('body');
                self.$modal.modal();
                var $slider_type = self.$modal.find("#slider_type"),
                ...
                ajax.jsonRpc('/theme_crafito/category_get_options', 'call', {}).done(function(res) {
    ...
    options.registry.theme_crafito_product_slider = options.Class.extend({
        start: function(editMode) {
            var self = this;
            this._super();
            this.$target.removeClass("o_hidden");
        ...
        prod_slider: function(type, value) {
            var self = this;
            if (type != undefined && type.type == "click" || type == undefined) {
                self.$modal = $(qweb.render("theme_crafito.crafito_dynamic_product_slider"));
                self.$modal.appendTo('body');
                self.$modal.modal();
                var $slider_type = self.$modal.find("#slider_type"),
                ...
                ajax.jsonRpc('/theme_crafito/product_get_options', 'call', {}).done(function(res) {
                    $('#slider_type option[value!="0"]').remove();
                    _.each(res, function(y) {
                        $("select[id='slider_type'").append($('<option>', {
                            value: y["id"],
    ...
    options.registry.theme_crafito_featured_product_slider = options.Class.extend({
        start: function(editMode) {
            var self = this;
            this._super();
            this.$target.removeClass("o_hidden");
        ...
        featured_prod_slider: function(type, value) {
            var self = this;
            if (type != undefined && type.type == "click" || type == undefined) {
                self.$modal = $(qweb.render("theme_crafito.crafito_dynamic_featured_product_slider"));
                self.$modal.appendTo('body');
                self.$modal.modal();
                ...
                ajax.jsonRpc('/theme_crafito/featured_product_get_options', 'call', {}).done(function(res) {
                    $('#slider_type option[value!="0"]').remove();
                    _.each(res, function(y) {
    ...
    options.registry.biztech_new_theme_fact_sheet_jsselector = options.Class.extend({
        ...
        biztech_fact_sheet_function: function(type) {
            if (type != undefined && type.type == "click" || type == undefined) {
                var self = this;
                var factcount
                self.$modal = $(qweb.render("theme_crafito.crafito_dynami_fact_sheet"));
                self.$modal.appendTo('body');
                self.$modal.modal();
    ...
    options.registry.biztech_new_theme_skill_jsselector = options.Class.extend({
        ...
        biztech_new_theme_skill_function: function(type) {
            if (type != undefined && type.type == "click" || type == undefined) {
                var self = this;
                var skillcount;
                self.$modal = $(qweb.render("theme_crafito.crafito_dyanamic_skill"));
                self.$modal.appendTo('body');
                self.$modal.modal();
    ...
    options.registry.theme_crafito_news1_js = options.Class.extend({
        start: function(editMode) {
            var self = this;
            this._super();
            this.$target.removeClass("hidden");
            this.$target.find('.crafito_newsone_slider').empty();
            if (!editMode) {
                self.$el.find(".crafito_newsone_slider").on("click", _.bind(self.theme_crafito_newsone_slider, self));
            }
        },
        onBuilt: function() {
            var self = this;
            this._super();
            if (this.theme_crafito_newsone_slider()) {
                this.theme_crafito_newsone_slider().fail(function() {
                    self.getParent()._removeSnippet();
                });
            }
        },
        ...
        cleanForSave: function() {
            $('.crafito_newsone_slider').empty();
        },
        theme_crafito_newsone_slider: function(type, value) {
            var self = this;
            if (type != undefined && type.type == "click" || type == undefined) {
                self.$modal = $(qweb.render("theme_crafito.crafito_blog_slider_block"));
                self.$modal.appendTo('body');
    ...
    options.registry.theme_crafito_news2_js = options.Class.extend({
        start: function(editMode) {
            var self = this;
            this._super();
            this.$target.removeClass("o_hidden");
            this.$target.find('.crafito_newstwo_slider').empty();
            if (!editMode) {
                self.$el.find(".crafito_newstwo_slider").on("click", _.bind(self.theme_crafito_newstwo_slider, self));
            }
        },
        onBuilt: function() {
            var self = this;
            this._super();
            if (this.theme_crafito_newstwo_slider()) {
                this.theme_crafito_newstwo_slider().fail(function() {
                    self.getParent()._removeSnippet();
                });
            }
        },
        ...
        cleanForSave: function() {
            $('.crafito_newstwo_slider').empty();
        },

        theme_crafito_newstwo_slider: function(type, value) {
            var self = this;
            if (type != undefined && type.type == "click" || type == undefined) {
                self.$modal = $(qweb.render("theme_crafito.crafito_blog_slider_block"));
                self.$modal.appendTo('body');
    ...
    options.registry.theme_crafito_hardware_blog_snippet = options.Class.extend({
        start: function(editMode) {
            var self = this;
            this._super();
            this.$target.removeClass("o_hidden");
            this.$target.find('.theme_crafito_new_blog').empty();
            if (!editMode) {
                self.$el.find(".theme_crafito_new_blog").on("click", _.bind(self.theme_crafito_new_blog_function, self));
            }
        },
        onBuilt: function() {
            var self = this;
            this._super();
            if (this.theme_crafito_new_blog_function()) {
                this.theme_crafito_new_blog_function().fail(function() {
                    self.getParent()._removeSnippet();
                });
            }
        },
        ...
        cleanForSave: function() {
            $('.theme_crafito_new_blog').empty();
        },

        theme_crafito_new_blog_function: function(type, value) {
            var self = this;
            if (type != undefined && type.type == "click" || type == undefined) {
                self.$modal = $(qweb.render("theme_crafito.crafito_blog_slider_block"));
                self.$modal.appendTo('body');
    ...
    options.registry.theme_crafito_faq_three_tabing = options.Class.extend({
        start: function() {
            var self = this;
    ...
    options.registry.theme_crafito_faq_one_tabing = options.Class.extend({
        start: function() {
            var self = this;
